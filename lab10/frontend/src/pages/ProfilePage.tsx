    ...
    <div style={{ maxWidth: 500, margin: '4rem auto', padding: '2rem', border: '1px solid #ccc', borderRadius: 8 }}>
      ...
      {data && (
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <tbody>
            <tr>
              <td style={{ padding: '0.5rem', fontWeight: 'bold', width: '40%' }}>ID:</td>
              <td style={{ padding: '0.5rem' }}>{data.id}</td>
            </tr>
            ...
            <tr>
              <td style={{ padding: '0.5rem', fontWeight: 'bold' }}>Дата реєстрації:</td>
              <td style={{ padding: '0.5rem' }}>{new Date(data.createdAt).toLocaleDateString('uk-UA')}</td>
            </tr>
          </tbody>
        </table>
      )}
      <button onClick={handleLogout} style={{ marginTop: '1.5rem', padding: '0.6rem 1.5rem' }}>
        Вийти
      </button>
    </div>
  )

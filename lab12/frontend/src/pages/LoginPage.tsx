...
export function LoginPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (data) => { await login(data.access_token); navigate('/profile') },
  })

  const getMsg = (e: any) => {
    const s = e?.response?.status
    if (s === 401) return 'Невірний email або пароль'
  ...
  return (
    <div style={{ maxWidth: 400, margin: '4rem auto', padding: '2rem', border: '1px solid #ccc', borderRadius: 8 }}>
      ...
      <p style={{ marginTop: '1rem', textAlign: 'center' }}>Немає акаунта? <Link to="/register">Зареєструватися</Link></p>
    </div>
  )
}

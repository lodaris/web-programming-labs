...
const INITIAL_TASKS: Task[] = [
  { id: `task-${VARIANT}-1`, title: `Задача А-${VARIANT}: налаштування середовища`,
    description: 'Встановити Node.js, VS Code та необхідні розширення',
    status: 'done', priority: 'high',
    createdAt: new Date(2025, 0, (VARIANT % 28) + 1) },
  { id: `task-${VARIANT}-2`, title: `Задача Б-${VARIANT}: вивчення документації`,
    description: 'Ознайомитись з офіційною документацією React',
    status: 'in-progress', priority: 'medium',
    createdAt: new Date(2025, 1, (VARIANT % 28) + 1) },
  { id: `task-${VARIANT}-3`, title: `Задача В-${VARIANT}: написати компонент`,
    description: '', status: 'todo', priority: 'low',
...
function App() {
  ...
  const handleAddTask = (data: TaskFormData) => {
    const newTask: Task = { id: crypto.randomUUID(), ...data, status: 'todo', createdAt: new Date() };
    ...
    <div className={styles.app}>
      <header className={styles.header}>
        ...
        <p className={styles.stats}>
          Всього: {tasks.length} | Нові: {tasks.filter((t) => t.status === 'todo').length} |
          В роботі: {tasks.filter((t) => t.status === 'in-progress').length} |
          Виконані: {tasks.filter((t) => t.status === 'done').length}
        </p>
      </header>
      <main className={styles.main}>
        <aside className={styles.sidebar}><TaskForm onSubmit={handleAddTask} /></aside>
